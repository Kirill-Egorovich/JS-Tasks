...
если возраст кошки <= 2 года, человеческий возраст = возраст кошки * 10. Если кошке больше 2 лет, человеческий возраст = возраст кошки * 7. (сделать через map)
...
function getAverageHumanAge(arr){
    const humanAges = arr.map(item => item <= 2 ? item * 10 : item * 7);

    const filterAges = humanAges.filter(item => item >= 18);

    const averageHumanAge = filterAges.reduce((acc, item) => acc + item, 0) / filterAges.length;
